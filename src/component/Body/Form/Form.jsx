...
export const Form = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_q6sg8f8', 'template_j0n93nb', form.current, 'xSWgWwOwsnm9kxc3a')
      .then((result) => {
        ...
        <form ref={form} onSubmit={sendEmail} className={styels.formBlock}>
          <div className={styels.form}>
            ...
            <div className={styels.wrapTextArea}>
              ...
              <textarea className={styels.textArea} name="message" id="" cols="30" rows="10"></textarea>
            </div>
            <button className={styels.button} type="submit">Отрправить заявку</button>
          </div>
        </form>
      </div>
    </div>
